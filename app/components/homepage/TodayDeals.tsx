...
const TodayDeals = ({product}: {product: any}) => {
  ...
  if (isValidArray(product.nodes)) {
    [...product.nodes, ...product.nodes, ...product.nodes].forEach((item) => {
      slides.push(
        <SwiperSlide className="w-34 h-[213px] rounded bg-white">
  ...
  return (
    <div className="w-full">
      <div className="w-full pt-4 pb-8 px-2 tablet:px-4 tablet:py-2 desk-pri:pt-10 desk-pri:px-20 desk-pri:pb-0 desk-sec:py-10 desk-sec:px-40 flex flex-col tablet:gap-y-1 desk-pri:gap-y-2 desk-pri:py-10
      desk-pri:h-[26.875rem]
      ">
